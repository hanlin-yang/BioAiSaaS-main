...
- **Core Library (`bioaisaas/`)**: Contains main application logic, including:
  - **Agent framework (`bioaisaas/agent/`)**: Implements autonomous AI agents using LLMs and workflow graphs.
  - **Task definitions (`bioaisaas/task/`)**: Abstract base and concrete biomedical benchmark tasks.
  - **Tool implementations (`bioaisaas/tool/`)**: Domain-specific analysis functions, API clients, and computational biology workflows.
  - **Tool metadata (`bioaisaas/tool/tool_description/`)**: Declarative schemas describing tool APIs and parameters.
  - **Model components (`bioaisaas/model/`)**: AI-driven resource retriever for selecting relevant tools and data.
  - **Utility modules (`bioaisaas/utils.py`, `bioaisaas/llm.py`, `bioaisaas/env_desc.py`)**: Helpers for LLM instantiation, system commands, environment descriptions.
  - **Versioning (`bioaisaas/version.py`)**: Package version management.

- **Environment Setup (`bioaisaas_env/`)**: Scripts and configuration files for reproducible environment provisioning, including:
...
- **Scripts (`bioaisaas/biorxiv_scripts/`)**: Data processing pipelines for literature mining and task extraction.
...
#### `bioaisaas/tool/tool_description/`
...
#### `bioaisaas/utils.py` and `bioaisaas/llm.py`
...
### Environment Setup (`bioaisaas_env/`)
...
- **Agent usage**: Instantiate `react` agent from `bioaisaas.agent.react`, configure with tools and retrieval, then call `go(prompt)` to run reasoning workflows.
- **Task evaluation**: Use classes in `bioaisaas.task` to load datasets, generate prompts, and evaluate LLM outputs.
- **Tool invocation**: Call functions in `bioaisaas.tool` modules or use API facades in `database.py` to query external resources.
...
- **Environment setup**: Run `bioaisaas_env/setup.sh` to provision environment and install dependencies.
...
- **Abstract base classes**: Used in `bioaisaas.task.base_task` to enforce consistent task interfaces.
...
1. **Environment Setup**
   - Run `bioaisaas_env/setup.sh` to create the Conda environment, install R packages, and CLI tools.
   - Source `bioaisaas_env/setup_path.sh` or add it to your shell profile to configure PATH.

2. **API Keys**
   - Set environment variables `OPENAI_API_KEY` and/or `ANTHROPIC_API_KEY` for LLM access.

3. **Running Agents**
   - Import and instantiate the `react` agent from `bioaisaas.agent.react`.
   - Configure with desired tools and retrieval options.
   - Call `go(prompt)` to execute reasoning workflows.

4. **Executing Tasks**
   - Use classes in `bioaisaas.task` to load datasets and evaluate LLM responses.
   - Implement new tasks by subclassing `base_task` and following the interface.

5. **Querying Databases**
   - Use `bioaisaas.tool.database` functions (e.g., `query_uniprot`, `query_gwas_catalog`) to retrieve data via natural language or direct parameters.

6. **Extending Tools**
   - Add new tool metadata in `bioaisaas/tool/tool_description/` as structured dictionaries.
   - Implement corresponding analysis functions in `bioaisaas/tool/`.
